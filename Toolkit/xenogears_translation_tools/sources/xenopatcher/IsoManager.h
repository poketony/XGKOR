...
public:
	...
	int Append(unsigned char *buffer, unsigned long size, int lastSector = 0);
	int Write(unsigned char* buffer, unsigned long sector, unsigned long size, int lastSector = 0);
	int CreateIso(const char* filename);
	...
	int OpenIso(const char* filename);
