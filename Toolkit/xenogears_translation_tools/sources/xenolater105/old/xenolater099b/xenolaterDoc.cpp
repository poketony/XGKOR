...
BOOL CXenolaterDoc::MainFileDecrypter()
{
	...
	//Se la lunghezza del file è minore di 50 ritorna
	if (m_nDecompLength < 50){
...
BOOL CXenolaterDoc::MainFileEncrypter()
{
	...
	//Copia anche i succesivi 2 byte (primo pointer)
	//memcpy(pTempData + 4, m_pDecompData + 4, 2);
	...
	for (UINT nCurrentBlock = 0; nCurrentBlock < wNumOfBlocks; nCurrentBlock++){
		...
		nStartPos++;
		INT max_width=0, max_height=0, width=0, height=0;	//Per calcolare max larghezza e altezza
		for(INT nIndex = nStartPos; nIndex < nEndPos; nIndex++){
			...
			else if(encrypt_Default(strText[nIndex], pTempData, (WORD*)&nTempOffset));
			...
			else if (strText[nIndex] == '<')
			{
				...
				//Altrimenti default
				else if (encrypt_Default(strTemp, pTempData,(WORD*) &nTempOffset));
				//Infine negli opcode
				else if (encrypt_OpCodes(strTemp, pTempData, (WORD*)&nTempOffset));
				//Altrimenti Errore!
				else
				{
					CString strErr;
					strErr.Format("Can't find appropiate code for %s", strTemp);
			...
			{
				CString strTmpDft;
				strTmpDft.Format("Unsupported char detected %c", strText[nIndex]);
